...
- **Signature visual moment**：最后一枚字钉落下时，散落在环、球与印版上的两行字从多个角度瞬间对齐，成为悬浮的最终 byline。
...
- **Camera and perspective**：透视相机 `42°`，主体位于画面中央 `18%–78%` 高度；单指空白拖动在受控范围内环绕。
- **Playfield focal area**：中央 70% 宽度为几何和字钉；字钉初始分布形成三角阅读路径。
- **Foreground / midground / background**：前景为偶尔越过镜头的薄印版边缘，中景为投影主体，背景为消失于黑色的地平网格与低密度雾。
- **HUD safe areas**：顶部 `max(18px, env(safe-area-inset-top))` 起；底部操作距 home indicator 至少 `20px`。不为外部访客栏下移构图。
- **Attention path**：用户名 HUD → 中央亮字钉 → 被切开的排版 → 下一枚字钉 / 终局署名。
...
- **Dominant shapes**：薄印版、窄拱门、折页、环与球；直角为主，只有字钉与轨道使用圆形。
- **Borders/shadows**：UI 使用 1px 纸白 18% 边框；不使用通用大圆角；阴影仅用于投影机般的硬边落影。
- **Materials**：哑黑金属、半粗糙乳白陶瓷、带加色投影的深灰印版。
...
- **States**：加载为扫描线；按下字钉缩进并出现外圈；完成字钉同时显示实心中心与刻度；禁用降低对比并保留轮廓；错误为暖橙图标+文字。
- **Emoji policy**：功能 UI 永不使用 Emoji。

## 8. Motion and VFX

- **Motion tokens**：即时 `90ms`、常规 `220ms`、定型 `720ms`、场景切换 `420/620ms`；缓动 `cubic-bezier(.2,.8,.2,1)`。
- **Input response**：同帧字钉压入、投影扫描向触点聚拢；`90ms` 达峰，`260ms` 回弹。
...
- **Gameplay**：第 1 关三枚字钉均可触达；拖动空白区旋转相机而不误触；每次激活真实提高排版对齐度。
- **High-feedback moment**：第三枚字钉触发 `720ms` 几何与投影套准，终态排版比过程态更完整。
